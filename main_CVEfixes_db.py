...
from src.dataset import get_CVEfixes_labels, get_texts, vulDataset
...
FROM file_change f, commits c, fixes fx, cve cv, cwe_classification cc, method_change mc
WHERE f.hash = c.hash 
AND c.hash = fx.hash 
AND fx.cve_id = cv.cve_id 
AND cv.cve_id = cc.cve_id 
...
CVEfixes_df = pd.read_sql_query(small_query, conn)
print("Before dropping, # of total rows: ",CVEfixes_df.shape[0])
...
print("After dropping, # of total rows: ",df.shape[0])
#df = df.sample(frac=0.0001)
df = df.head(100)
print(f"# of CVEfixes {frac*100}% dataset rows: {df.shape[0]}")
...
def train_classification(df, model_name, EPOCH, class_type):
    text_col_name = 'code'
...
EPOCH = 1
train_classification(df, model_name, EPOCH, 'binary')
