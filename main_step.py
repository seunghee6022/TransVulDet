...
#import matplotlib.pyplot as plt
from src.dataset import OversampledDatasetGenerator
...
def evaluate(model, class_type, average, device, criterion, val_dataloader, writer):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(val_dataloader):
            ...
            print("Eval - labels",labels.shape, labels)  
...
def objective(trial, train_df, val_df, test_df, output_dir, class_type, model_name, step_size):
    ...
    train_dataset = OversampledDatasetGenerator(train_df, tokenizer, text_col_name, label_col_name, class_type, num_labels)
    print("OversampledDatasetGenerator - Val START")
    val_dataset = OversampledDatasetGenerator(val_df, tokenizer, text_col_name, label_col_name, class_type, num_labels)
    ...
    train_losses, train_accs, train_bal_accs, train_precisions, train_recalls, train_f1s = train(model, class_type, average, device, criterion, optimizer, train_dataloader, writer,step_size)
